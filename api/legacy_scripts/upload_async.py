...
from api.utils import get_file_name_and_type, get_permissions
...
class BaseDocumentProcessor:
    def __init__(self, file_path):
        ...
        # Chunk details
        self._content: Union[str, None] = ""
        self._label_prompt: Union[str, None] = ""
        self._permissions: List[str] = ["admin"]
        self.file_path: str = file_path
        self.file_name: str = get_file_name_and_type(file_path)[0]
        self.file_type: str = get_file_name_and_type(file_path)[1]
    ...
    def _init_doc(self, doc_content) -> None:
        ...
        permissions = get_permissions(self.file_path)
    ...
    async def _upload_to_elasticsearch(self) -> None:
       ...
       # await self._remove_elasticsearch_content()
        ...
        docs: List[Document] = list(text_splitter.transform_documents(workspace_docs))

        try:
            response = ElasticsearchStore.from_documents(
                docs,
            ...
            print(f"{self.file_name} uploaded to Elasticsearch")
            print("Elasticsearch response:", response)
        except Exception as e:
            print(f"Error uploading {self.file_name} to Elasticsearch: {e}")
    ...
    async def _remove_elasticsearch_content(self) -> None:
        ...
        print("previous document has been removed")
        pass
...
class TxtProcessor(BaseDocumentProcessor):
    async def process(self) -> None:
        ...
        async with aiofiles.open(self.file_path, 'r', encoding='utf-8') as f:
            ...
            labels = eval(base_message.content)[self._label_key_name]  # gather labels
...
class PdfProcessor(BaseDocumentProcessor):
    ...
    async def process(self):
        ...
        self._init_doc(content)
        base_message: BaseMessage = await self._ask_llm_for_labels()
        labels = eval(base_message.content)[self._label_key_name]
        self._set_labels(labels)
        await self._upload_to_elasticsearch()
...
async def process_file(file_path):
    if file_path.endswith('.pdf'):
        pdf_processor = PdfProcessor(file_path=file_path)
